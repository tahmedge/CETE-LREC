...
class Dataset(object):
    ...
    def get_pandas_df(self, filename):
        ...
        with open(filename, 'r',encoding="utf8") as datafile:
            ...
            for line in datafile:
                ...
                res.append([int(lines[0]), lines[1], float(lines[2])])
            #return res
            #tempdata=[]
            #for line in datafile:
            #    tempdata = line.split('\t')
            ...
            data_label = list(map(lambda x: int(x[2]), data))
        #full_df = pd.DataFrame({"index":data_index,"text1":data_text1, "text2":data_text2, "label":data_label})
        full_df = pd.DataFrame({"index": data_index, "text": data_text, "label": data_label})
        return full_df
    '''
    def get_pandas_df(self, filename):
    ...
    def load_data(self, train_file, test_file=None, val_file=None):
        ...
        datafields = [("index", INDEX), ("text", TEXT), ("label", LABEL)]
